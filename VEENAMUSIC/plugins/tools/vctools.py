...
from VEENAMUSIC import app, Userbot
...
async def strcall(client, message):
    assistant = await group_assistant(VEENA, message.chat.id)
    try:
        await assistant.join_group_call(message.chat.id, AudioPiped("./VEENAMUSIC/assets/call.mp3"), stream_type=StreamType().pulse_stream)
